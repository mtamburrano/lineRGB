...
void readDirectory(string dirpath, vector<string>& list_files, bool searchDirectories) {


	for ( boost::filesystem::directory_iterator it( dirpath );
		it != boost::filesystem::directory_iterator(); ++it )
	{
		if (searchDirectories == false && boost::filesystem::is_regular_file( it->status() ) ) {
			list_files.push_back( (string)it->path().c_str());
		}
		else if (searchDirectories == true && boost::filesystem::is_directory( it->status() ) ) {
			list_files.push_back( (string)it->path().c_str());
		}
	}
...
TemplatesObject* getImagePathsFromListFile(vector<string>& list_files, string name, bool with_depth) {
...
void getAllObjectsTemplates(string templates_folder, vector<TemplatesObject*>& all_objects_templates, bool with_depth) {
    ...
    for(int i = 0; i<list_dirs.size(); i++) {
    	...
    	all_objects_templates.push_back(getImagePathsFromListFile(list_files,folder_name, with_depth));
    }
...
int main(int argc, char * argv[]) {
    ...
    } else if (line2d == false && linergb == false && linemodrgb == false && linemod == false) {
        printf("Please specify one modality: \"--linergb\" or \"--line2d\" or \"--linemodrgb\" or \"--linemod\"\n");
        printf("use -h option to show help\n");
        return 0;
    }
	...
	for (int no = 0; no < num_objects; no++) {
		...
		cout << "------ Training " << class_id << " with " <<num_images<< "images --------" << endl;
        ...
        for (int ni = 0; ni < num_images; ni++) {
			...
			if (linemod == true)
			{
				template_id = detector_linemod->addTemplate(
						sourcesTemplate, class_id, mask,
						&bb);
			}
			///////////DEBUG/////////////////////////////
			if(DEBUG == true)
			{
				...
				drawResponseLineRGB(templates,
				        num_modalities, rgb, cv::Point(bb.x, bb.y), T,
				        -1, class_id, only_non_border);
				/*
